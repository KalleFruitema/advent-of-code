...
for ri, line in enumerate(data):
    in_number = False
    for ci, val in enumerate(line):
        if val.isnumeric():
            bool_vals = []
            for ci2 in range(ci-1, ci+2):
                for ri2 in range(ri-1, ri+2):
                    try:
                        bool_vals.append(data[ri2][ci2] == '.' or data[ri2][ci2].isnumeric() )
                    except IndexError:
                        pass
            if all(bool_vals) and  not in_number:
                numlist.append([val])
                in_number = True
            elif all(bool_vals) and in_number:
                numlist[-1].append(val)
            elif not all(bool_vals) and not in_number:
                numlist.append(["x", val])
                in_number = True
            elif not all(bool_vals) and in_number:
                numlist[-1].extend(["x", val])
        else:
            in_number = False
...
for nums in numlist:
    if "x" in nums:
        total += int("".join([i for i in nums if i.isnumeric()]))
    ...
    for ci, val in enumerate(line):
        if val == "*":
            ...
            for ri2 in range(ri-1, ri+2):
                for ci2 in range(ci-1, ci+2):
                    try:
                        if data[ri2][ci2].isnumeric():
                            if not in_number:
                                in_number = True
                                found_num = data[ri2][ci2]
                                start_pos = (ri2, ci2)
                                star_nums.append((found_num,
                                             start_pos))
                        elif in_number:
                            found_num = start_pos = ""
                            in_number = False
                        ...
                        pass
                in_number = False
            for start_num, start_pos in star_nums:
                full_nums = []
                for loop in range(2):
                    full_num = ""
                    n_val = start_num
                    ...
                    while n_val.isnumeric():
                        if loop == 0:
                            full_num = f"{full_num}{n_val}"
                        elif loop == 1:
                            full_num = f"{n_val}{full_num}"
                        try:
                            n_val = data[ri3][ci3+count]
                        except IndexError:
                            break
                        if loop == 0:
                            count += 1
                ...
                try:
                    real_fn = f"{full_nums[1]}{full_nums[0][1:]}"
                except IndexError:
                    real_fn = full_nums[1]
                real_star_nums.append(real_fn)
            numlist2.append((real_star_nums))
                    
total = 0
for nums in numlist2:
    if len(nums) == 2:
