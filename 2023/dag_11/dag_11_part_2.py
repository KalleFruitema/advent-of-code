...
for y, line in enumerate(grid):
    for x, element in enumerate(line):
        if element == "#":
            exp_dist_y_count = [z[x] for z in grid][:y].count("*")
            exp_dist_x_count = line[:x].count("*")
            exp_dist_y = exp_dist_y_count * 1_000_000
            exp_dist_x = exp_dist_x_count * 1_000_000
            grid[y][x] = str(count)
            all_solar.append((str(count), (y - exp_dist_y_count + exp_dist_y, 
                                           x - exp_dist_x_count + exp_dist_x)))
            count += 1
