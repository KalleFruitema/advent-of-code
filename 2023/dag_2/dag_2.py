...
for key, val in dic.items():
    dic[key] = [i.strip().split(',') for i in val]

for key, val in dic.items():
    for i, _round in enumerate(val):
        dic[key][i] = [color.strip().split(" ") 
                       for color in _round]
red = 12 
green = 13
blue = 14
...
for game, rounds in dic.items():
    impossible = False
    for _round in rounds: 
        for amount, color in _round:
        ...
        if impossible:
            break
    if not impossible:
        totalsum.append(int(game.split(" ")[-1]))
    ...
    for _round in rounds:
        for amount, color in _round:
            if color == "red" and int(amount) > max_red:
                ...
                max_blue = int(amount)
    totals2.append(max_red * max_green * max_blue)
