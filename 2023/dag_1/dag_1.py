...
for i, y in enumerate(data):
    ...
    for char in y[::-1]:
        if char.isnumeric():
            num_list_rev.append(char)
            break
        if char.isalpha():
            char_str += char
            for num, val in num_dict.items():
                if num in char_str[::-1]:
                    num_list_rev.append(val)
                    breaker2 = True
        if breaker2:
            break
...
print(len(num_list), len(num_list_rev))

for i in range(len(num_list)):
    total += int(str(num_list[i]) + str(num_list_rev[i]))
