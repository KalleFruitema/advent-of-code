...
tas = tas.strip().splitlines()

# new_tas = []
# for i in tas:
#     # sets gebruiken?
#     comp1, comp2 = i[:int(len(i)/2)], i[int(len(i)/2):]
#     for i in comp1:
#         if i in comp2:
#             new_tas.append(i)
#             break
...
for i in tas[::3]:
    index = tas.index(i)
    groep = tas[index : index + 3 ]
    for i in groep[0]:
        if i in groep[1] and i in groep[2]:
            badge = i
            break
    new_tas.append(badge)
...
new_new_tas = []
for i in new_tas:
    new_new_tas.append(value.index(i))

print(sum(new_new_tas))